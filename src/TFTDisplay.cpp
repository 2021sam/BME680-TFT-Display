...
void TFTDisplay::showAllData(float temperature, float humidity, float pressure, float gas) {
    clearScreen();  // Clear the screen before displaying new data

    // Display sensor data on the screen
    drawText("Temp: " + String(temperature) + " C", 10, 10);
    drawText("Humidity: " + String(humidity) + " %", 10, 40);
    drawText("Pressure: " + String(pressure) + " hPa", 10, 70);
    drawText("Gas: " + String(gas) + " kOhms", 10, 100);
}
